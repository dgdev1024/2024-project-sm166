...
namespace smboy
{
  ...
  static_assert(sizeof(object) == 4, "sizeof(smboy::object) != 4!");

  /** Constructor and Destructor ******************************************************************/

  renderer::renderer (emulator& _emulator) :
    m_vram      { m_vram0 },
  ...
  void renderer::initialize ()
  {
    ...
    m_vram = m_vram0;
    m_vram0.clear();    m_vram0.resize(vram_size, 0x00);
    m_vram1.clear();    m_vram1.resize(vram_size, 0x00);
  ...
  void renderer::tick_oam_dma ()
  {
    ...
    if (dma_byte < 0xA0)
    {
      ...
      {
        std::uint8_t* oam_bytes = reinterpret_cast<std::uint8_t*>(m_oam.data());
        oam_bytes[dma_byte] = m_emulator.get_memory().read_byte(m_dma_source);
        m_dma_source++;
      }
    }
  }
